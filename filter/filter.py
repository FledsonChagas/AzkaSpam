...
SECRET_KEY_ENV = "AZKASPAM_SECRET_KEY"
SECRET_KEY_PATH = STATE_DIR / "azkaspam.secret_key"
...
REQUIRED_PER_ACCOUNT: tuple[str, ...] = ("name", "user", "imap_host")
...
def generate_secret_key() -> str:
    """Return a new Fernet key suitable for AZKASPAM_SECRET_KEY."""
    return Fernet.generate_key().decode("ascii")


def _secret_cipher() -> Fernet:
    raw = os.environ.get(SECRET_KEY_ENV, "").strip()
    if not raw and SECRET_KEY_PATH.is_file():
        raw = SECRET_KEY_PATH.read_text().strip()
    if not raw:
        raise SystemExit(
            f"{SECRET_KEY_ENV} is required to decrypt IMAP credentials. "
            "Generate one with `python filter.py generate-secret-key` and "
    ...
    try:
        ...
        raise SystemExit(
            f"{SECRET_KEY_ENV} is not a valid Fernet key; generate a new one "
            "with `python filter.py generate-secret-key`."
        ) from ex


def encrypt_config_secret(plaintext: str) -> str:
    if not plaintext:
        raise SystemExit("cannot encrypt an empty IMAP password")
    token = _secret_cipher().encrypt(plaintext.encode("utf-8")).decode("ascii")
    return ENCRYPTED_SECRET_PREFIX + token


def decrypt_config_secret(encrypted: Any, account_name: str) -> str:
    value = str(encrypted or "").strip()
    if not value:
        raise SystemExit(f"account {account_name!r}: password_encrypted is required")
    if not value.startswith(ENCRYPTED_SECRET_PREFIX):
        raise SystemExit(
            f"account {account_name!r}: password_encrypted must start with "
    ...
    except (InvalidToken, UnicodeDecodeError, ValueError) as ex:
        raise SystemExit(
            f"account {account_name!r}: password_encrypted could not be decrypted"
        ) from ex


def _account_password(merged: dict[str, Any], account_name: str) -> str:
    if merged.get("password"):
        raise SystemExit(
            f"account {account_name!r}: plaintext password is not supported. "
            "Use password_encrypted generated by `python filter.py encrypt-password`."
        )
    if not merged.get("password_encrypted"):
        raise SystemExit(
            f"account {account_name!r}: missing required key password_encrypted"
        )
    return decrypt_config_secret(merged["password_encrypted"], account_name)
...
def load_accounts(path: Path) -> list[Account]:
    ...
    for entry in raw["accounts"]:
        ...
        try:
            account_name = merged.get("name", "?")
            acc = Account(
                name=merged["name"],
                user=merged["user"],
                password=_account_password(merged, account_name),
...
def _cli() -> int | None:
    if len(sys.argv) < 2:
        return None
    cmd = sys.argv[1]
    if cmd == "generate-secret-key":
        print(generate_secret_key())
        return 0
    if cmd == "encrypt-password":
        if len(sys.argv) >= 3:
            raise SystemExit(
                "encrypt-password does not accept the password as an argument; "
                "use the interactive prompt to avoid shell history leaks"
            )
        password = getpass.getpass("IMAP password: ")
        print(encrypt_config_secret(password))
        return 0
    return None
...
if __name__ == "__main__":
    cli_result = _cli()
    sys.exit(main() if cli_result is None else cli_result)
